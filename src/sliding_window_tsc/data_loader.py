...
def find_train_test_files(dataset_folder: str | Path):
    """Find the single TRAIN and TEST .ts files inside a dataset folder."""
    ...
    if len(train_files) != 1:
        raise FileNotFoundError(
            f"Expected exactly one *_TRAIN.ts file in {dataset_path}, "
            f"found {len(train_files)}."
        )

    if len(test_files) != 1:
        raise FileNotFoundError(
            f"Expected exactly one *_TEST.ts file in {dataset_path}, "
...
def load_dataset(dataset_folder: str | Path):
    """Load train and test data from a dataset folder containing .ts files."""
